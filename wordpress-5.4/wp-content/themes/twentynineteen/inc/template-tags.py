...
if (not php_function_exists("twentynineteen_posted_on")):
    ...
    #//
    def twentynineteen_posted_on(*args_):
        
        time_string = "<time class=\"entry-date published updated\" datetime=\"%1$s\">%2$s</time>"
        if get_the_time("U") != get_the_modified_time("U"):
            time_string = "<time class=\"entry-date published\" datetime=\"%1$s\">%2$s</time><time class=\"updated\" datetime=\"%3$s\">%4$s</time>"
        # end if
        time_string = php_sprintf(time_string, esc_attr(get_the_date(DATE_W3C)), esc_html(get_the_date()), esc_attr(get_the_modified_date(DATE_W3C)), esc_html(get_the_modified_date()))
...
if (not php_function_exists("twentynineteen_posted_by")):
    ...
    #//
    def twentynineteen_posted_by(*args_):
...
if (not php_function_exists("twentynineteen_comment_count")):
    ...
    #//
    def twentynineteen_comment_count(*args_):
...
if (not php_function_exists("twentynineteen_entry_footer")):
    ...
    #//
    def twentynineteen_entry_footer(*args_):
        ...
        if "post" == get_post_type():
            ...
            if categories_list:
                printf("<span class=\"cat-links\">%1$s<span class=\"screen-reader-text\">%2$s</span>%3$s</span>", twentynineteen_get_icon_svg("archive", 16), __("Posted in", "twentynineteen"), categories_list)
                pass
            # end if
            #// translators: Used between list items, there is a space after the comma.
            tags_list = get_the_tag_list("", __(", ", "twentynineteen"))
            if tags_list:
                printf("<span class=\"tags-links\">%1$s<span class=\"screen-reader-text\">%2$s </span>%3$s</span>", twentynineteen_get_icon_svg("tag", 16), __("Tags:", "twentynineteen"), tags_list)
                pass
            # end if
        # end if
        #// Comment count.
...
if (not php_function_exists("twentynineteen_post_thumbnail")):
    ...
    #//
    def twentynineteen_post_thumbnail(*args_):
...
if (not php_function_exists("twentynineteen_get_user_avatar_markup")):
    ...
    #//
    def twentynineteen_get_user_avatar_markup(id_or_email=None, *args_):
        
        if (not (php_isset(lambda : id_or_email))):
            id_or_email = get_current_user_id()
        # end if
        return php_sprintf("<div class=\"comment-user-avatar comment-author vcard\">%s</div>", get_avatar(id_or_email, twentynineteen_get_avatar_size()))
...
if (not php_function_exists("twentynineteen_discussion_avatars_list")):
    ...
    #//
    def twentynineteen_discussion_avatars_list(comment_authors=None, *args_):
        
        if php_empty(lambda : comment_authors):
            return
        # end if
        php_print("<ol class=\"discussion-avatar-list\">", "\n")
        for id_or_email in comment_authors:
...
if (not php_function_exists("twentynineteen_comment_form")):
    ...
    #//
    def twentynineteen_comment_form(order=None, *args_):
        
        if True == order or php_strtolower(order) == php_strtolower(get_option("comment_order", "asc")):
...
if (not php_function_exists("twentynineteen_the_posts_navigation")):
    ...
    #// Documentation for function.
    #//
    def twentynineteen_the_posts_navigation(*args_):
