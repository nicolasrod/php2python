...
if comments_:
    ...
    """)
    comments_number_ = absint(get_comments_number())
    php_print("""
    ...
    comment_pagination_ = paginate_comments_links(Array({"echo": False, "end_size": 0, "mid_size": 0, "next_text": __("Newer Comments", "twentytwenty") + " <span aria-hidden=\"true\">&rarr;</span>", "prev_text": "<span aria-hidden=\"true\">&larr;</span> " + __("Older Comments", "twentytwenty")}))
    if comment_pagination_:
        pagination_classes_ = ""
        #// If we're only showing the "Next" link, add a class indicating so.
        if False == php_strpos(comment_pagination_, "prev page-numbers"):
            pagination_classes_ = " only-next"
        # end if
        php_print("\n               <nav class=\"comments-pagination pagination")
        php_print(pagination_classes_)
        pass
        php_print("\" aria-label=\"")
        esc_attr_e("Comments", "twentytwenty")
        php_print("\">\n                    ")
        php_print(wp_kses_post(comment_pagination_))
        php_print("             </nav>\n\n              ")
    # end if
    php_print("""
    </div><!-- .comments-inner -->
