...
#//
twentytwenty_unique_id_ = twentytwenty_unique_id("search-form-")
twentytwenty_aria_label_ = "aria-label=\"" + esc_attr(args_["label"]) + "\"" if (not php_empty(lambda : args_["label"])) else ""
php_print("<form role=\"search\" ")
php_print(twentytwenty_aria_label_)
pass
php_print(" method=\"get\" class=\"search-form\" action=\"")
php_print(esc_url(home_url("/")))
php_print("\">\n    <label for=\"")
php_print(esc_attr(twentytwenty_unique_id_))
