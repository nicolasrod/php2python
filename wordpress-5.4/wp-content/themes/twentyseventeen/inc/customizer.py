...
#//
def twentyseventeen_customize_register(wp_customize=None, *args_):
    ...
    wp_customize.add_control(php_new_class("WP_Customize_Color_Control", lambda : WP_Customize_Color_Control(wp_customize, "colorscheme_hue", Array({"mode": "hue", "section": "colors", "priority": 6}))))
    #// 
    #// Theme options.
    #//
    ...
    while i < 1 + num_sections:
        ...
        wp_customize.add_control("panel_" + i, Array({"label": php_sprintf(__("Front Page Section %d Content", "twentyseventeen"), i), "description": "" if 1 != i else __("Select pages to feature in each area from the dropdowns. Add an image to a section by setting a featured image in the page editor. Empty sections will not be displayed.", "twentyseventeen"), "section": "theme_options", "type": "dropdown-pages", "allow_addition": True, "active_callback": "twentyseventeen_is_static_front_page"}))
...
#//
def twentyseventeen_sanitize_page_layout(input=None, *args_):
    
    valid = Array({"one-column": __("One Column", "twentyseventeen"), "two-column": __("Two Column", "twentyseventeen")})
    if php_array_key_exists(input, valid):
        return input
    # end if
    return ""
...
#//
def twentyseventeen_sanitize_colorscheme(input=None, *args_):
    
    valid = Array("light", "dark", "custom")
    if php_in_array(input, valid, True):
        return input
    # end if
    return "light"
...
#// @return void
#//
def twentyseventeen_customize_partial_blogname(*args_):
...
#// @return void
#//
def twentyseventeen_customize_partial_blogdescription(*args_):
...
#//
def twentyseventeen_is_static_front_page(*args_):
...
#//
def twentyseventeen_is_view_with_layout_option(*args_):
...
#//
def twentyseventeen_customize_preview_js(*args_):
...
#//
def twentyseventeen_panels_js(*args_):
