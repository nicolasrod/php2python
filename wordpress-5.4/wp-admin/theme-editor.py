...
if theme_.errors() and "theme_no_stylesheet" == theme_.errors().get_error_code():
    wp_die(__("The requested theme does not exist.") + " " + theme_.errors().get_error_message())
# end if
allowed_files_ = Array()
style_files_ = Array()
has_templates_ = False
file_types_ = wp_get_theme_file_editable_extensions(theme_)
for type_ in file_types_:
    for case in Switch(type_):
        if case("php"):
            allowed_files_ += theme_.get_files("php", -1)
            has_templates_ = (not php_empty(lambda : allowed_files_))
            break
        # end if
        if case("css"):
            style_files_ = theme_.get_files("css", -1)
            allowed_files_["style.css"] = style_files_["style.css"]
            allowed_files_ += style_files_
            break
        # end if
        if case():
            allowed_files_ += theme_.get_files(type_, -1)
            break
        # end if
    # end for
# end for
#// Move functions.php and style.css to the top.
if (php_isset(lambda : allowed_files_["functions.php"])):
    allowed_files_ = Array({"functions.php": allowed_files_["functions.php"]}) + allowed_files_
# end if
if (php_isset(lambda : allowed_files_["style.css"])):
    allowed_files_ = Array({"style.css": allowed_files_["style.css"]}) + allowed_files_
# end if
if php_empty(lambda : file_):
    relative_file_ = "style.css"
    file_ = allowed_files_["style.css"]
else:
    relative_file_ = wp_unslash(file_)
    file_ = theme_.get_stylesheet_directory() + "/" + relative_file_
# end if
validate_file_to_edit(file_, allowed_files_)
...
if "POST" == PHP_SERVER["REQUEST_METHOD"]:
    ...
    if is_wp_error(r_):
        edit_error_ = r_
        if check_ajax_referer("edit-theme_" + stylesheet_ + "_" + relative_file_, "nonce", False) and (php_isset(lambda : PHP_POST["newcontent"])):
            posted_content_ = wp_unslash(PHP_POST["newcontent"])
        # end if
    else:
        wp_redirect(add_query_arg(Array({"a": 1, "theme": stylesheet_, "file": relative_file_}), admin_url("theme-editor.php")))
        php_exit(0)
    # end if
# end if
...
elif (not error_) and filesize(file_) > 0:
    f_ = fopen(file_, "r")
    content_ = fread(f_, filesize(file_))
    if ".php" == php_substr(file_, php_strrpos(file_, ".")):
...
# end if
file_description_ = get_file_description(relative_file_)
file_show_ = php_array_search(file_, php_array_filter(allowed_files_))
description_ = esc_html(file_description_)
if file_description_ != file_show_:
    description_ += " <span>(" + esc_html(file_show_) + ")</span>"
# end if
...
for a_stylesheet_,a_theme_ in wp_get_themes(Array({"errors": None})):
    if a_theme_.errors() and "theme_no_stylesheet" == a_theme_.errors().get_error_code():
        continue
    # end if
    selected_ = " selected=\"selected\"" if a_stylesheet_ == stylesheet_ else ""
    php_print("\n   " + "<option value=\"" + esc_attr(a_stylesheet_) + "\"" + selected_ + ">" + a_theme_.display("Name") + "</option>")
# end for
php_print("     </select>\n     ")
...
""")
if theme_.errors():
    php_print("<div class=\"error\"><p><strong>" + __("This theme is broken.") + "</strong> " + theme_.errors().get_error_message() + "</p></div>")
# end if
...
wp_print_theme_file_tree(wp_make_theme_file_tree(allowed_files_))
php_print("""           </ul>
</li>
</ul>
</div>
...
if error_:
    ...
    wp_nonce_field("edit-theme_" + stylesheet_ + "_" + relative_file_, "nonce")
    ...
    php_print(esc_attr(relative_file_))
    ...
    """)
    if is_child_theme() and theme_.get_stylesheet() == get_template():
        php_print("                 <div class=\"notice notice-warning inline\">\n                      <p>\n                           ")
        if is_writeable(file_):
            php_print("                             <strong>")
            _e("Caution:")
            php_print("</strong>\n                          ")
        # end if
...
if (not php_in_array("theme_editor_notice", dismissed_pointers_, True)):
    #// Get a back URL.
    referer_ = wp_get_referer()
    excluded_referer_basenames_ = Array("theme-editor.php", "wp-login.php")
