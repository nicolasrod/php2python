...
if (not php_empty(lambda : link_id_)):
    ...
    heading_ = php_sprintf(__("<a href=\"%s\">Links</a> / Edit Link"), "link-manager.php")
    submit_text_ = __("Update Link")
    form_name_ = "editlink"
    nonce_action_ = "update-bookmark_" + link_id_
else:
    #// translators: %s: URL to Links screen.
    heading_ = php_sprintf(__("<a href=\"%s\">Links</a> / Add New Link"), "link-manager.php")
    submit_text_ = __("Add Link")
    form_name_ = "addlink"
    nonce_action_ = "add-bookmark"
# end if
...
do_action("add_meta_boxes", "link", link_)
#// 
...
#//
do_action("add_meta_boxes_link", link_)
#// This action is documented in wp-admin/includes/meta-boxes.php
do_action("do_meta_boxes", "link", "normal", link_)
#// This action is documented in wp-admin/includes/meta-boxes.php
do_action("do_meta_boxes", "link", "advanced", link_)
#// This action is documented in wp-admin/includes/meta-boxes.php
do_action("do_meta_boxes", "link", "side", link_)
...
# end if
php_print("\n<form name=\"")
php_print(esc_attr(form_name_))
php_print("\" id=\"")
php_print(esc_attr(form_name_))
php_print("\" method=\"post\" action=\"link.php\">\n")
if (not php_empty(lambda : link_added_)):
    php_print(link_added_)
# end if
...
php_print(esc_attr(link_.link_name))
...
php_print(esc_attr(link_.link_url))
...
php_print(esc_attr(link_.link_description) if (php_isset(lambda : link_.link_description)) else "")
...
side_meta_boxes_ = do_meta_boxes("link", "side", link_)
php_print("</div>\n<div id=\"postbox-container-2\" class=\"postbox-container\">\n")
do_meta_boxes(None, "normal", link_)
do_meta_boxes(None, "advanced", link_)
php_print("</div>\n")
if link_id_:
    php_print("<input type=\"hidden\" name=\"action\" value=\"save\" />\n<input type=\"hidden\" name=\"link_id\" value=\"")
    php_print(php_int(link_id_))
    php_print("\" />\n<input type=\"hidden\" name=\"cat_id\" value=\"")
    php_print(php_int(cat_id_))
    php_print("\" />\n")
else:
