...
for table,prefixed_table in wpdb.tables("ms_global"):
...
if PHP_POST:
    ...
    install_network()
    base = php_parse_url(trailingslashit(get_option("home")), PHP_URL_PATH)
    subdomain_install = (not php_empty(lambda : PHP_POST["subdomain_install"])) if allow_subdomain_install() else False
    if (not network_domain_check()):
        result = populate_network(1, get_clean_basedomain(), sanitize_email(PHP_POST["email"]), wp_unslash(PHP_POST["sitename"]), base, subdomain_install)
        if is_wp_error(result):
            if 1 == php_count(result.get_error_codes()) and "no_wildcard_dns" == result.get_error_code():
