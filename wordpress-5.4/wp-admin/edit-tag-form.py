...
if "category" == taxonomy_:
    ...
    #//
    do_action_deprecated("edit_category_form_pre", Array(tag_), "3.0.0", "{$taxonomy}_pre_edit_form")
    ...
    #//
    do_action_deprecated("edit_link_category_form_pre", Array(tag_), "3.0.0", "{$taxonomy}_pre_edit_form")
else:
    #// 
    ...
    #//
    do_action_deprecated("edit_tag_form_pre", Array(tag_), "3.0.0", "{$taxonomy}_pre_edit_form")
# end if
#// 
#// Use with caution, see https://developer.wordpress.org/reference/functions/wp_reset_vars
#//
wp_reset_vars(Array("wp_http_referer"))
wp_http_referer_ = remove_query_arg(Array("action", "message", "tag_ID"), wp_http_referer_)
...
#//
do_action(str(taxonomy_) + str("_pre_edit_form"), tag_, taxonomy_)
php_print("\n<div class=\"wrap\">\n<h1>")
php_print(tax_.labels.edit_item)
php_print("</h1>\n\n")
class_ = "error" if (php_isset(lambda : msg_)) and 5 == msg_ else "success"
if message_:
    ...
    if wp_http_referer_:
        php_print(" <p><a href=\"")
        php_print(esc_url(wp_validate_redirect(esc_url_raw(wp_http_referer_), admin_url("term.php?taxonomy=" + taxonomy_))))
        php_print("\">\n        ")
        php_print(esc_html(tax_.labels.back_to_items))
        php_print(" </a></p>\n  ")
    # end if
    php_print("</div>\n ")
# end if
...
#// @since 3.7.0
#//
do_action(str(taxonomy_) + str("_term_edit_form_tag"))
php_print(">\n<input type=\"hidden\" name=\"action\" value=\"editedtag\"/>\n<input type=\"hidden\" name=\"tag_ID\" value=\"")
php_print(esc_attr(tag_ID_))
php_print("\"/>\n<input type=\"hidden\" name=\"taxonomy\" value=\"")
php_print(esc_attr(taxonomy_))
php_print("\"/>\n")
wp_original_referer_field(True, "previous")
wp_nonce_field("update-tag_" + tag_ID_)
#// 
...
#// @param string  $taxonomy Current $taxonomy slug.
#//
do_action(str(taxonomy_) + str("_term_edit_form_top"), tag_, taxonomy_)
tag_name_value_ = ""
if (php_isset(lambda : tag_.name)):
    tag_name_value_ = esc_attr(tag_.name)
# end if
...
if (not global_terms_enabled()):
    ...
    #//
    slug_ = apply_filters("editable_slug", tag_.slug, tag_) if (php_isset(lambda : tag_.slug)) else ""
...
if is_taxonomy_hierarchical(taxonomy_):
    ...
    php_print("</label></th>\n          <td>\n              ")
    dropdown_args_ = Array({"hide_empty": 0, "hide_if_empty": False, "taxonomy": taxonomy_, "name": "parent", "orderby": "name", "selected": tag_.parent, "exclude_tree": tag_.term_id, "hierarchical": True, "show_option_none": __("None")})
    #// This filter is documented in wp-admin/edit-tags.php
    dropdown_args_ = apply_filters("taxonomy_parent_dropdown_args", dropdown_args_, taxonomy_, "edit")
...
if "category" == taxonomy_:
    ...
    #// @param WP_Term $tag Current category term object.
    #//
    do_action_deprecated("edit_category_form_fields", Array(tag_), "3.0.0", "{$taxonomy}_edit_form_fields")
elif "link_category" == taxonomy_:
    #// 
    ...
    #// @param WP_Term $tag Current link category term object.
    #//
    do_action_deprecated("edit_link_category_form_fields", Array(tag_), "3.0.0", "{$taxonomy}_edit_form_fields")
else:
    #// 
    ...
    #// @param WP_Term $tag Current tag term object.
    #//
    do_action_deprecated("edit_tag_form_fields", Array(tag_), "3.0.0", "{$taxonomy}_edit_form_fields")
# end if
#// 
...
#// @param string  $taxonomy Current taxonomy slug.
#//
do_action(str(taxonomy_) + str("_edit_form_fields"), tag_, taxonomy_)
php_print(" </table>\n")
#// Back compat hooks.
if "category" == taxonomy_:
    #// This action is documented in wp-admin/edit-tags.php
    do_action_deprecated("edit_category_form", Array(tag_), "3.0.0", "{$taxonomy}_add_form")
elif "link_category" == taxonomy_:
    #// This action is documented in wp-admin/edit-tags.php
    do_action_deprecated("edit_link_category_form", Array(tag_), "3.0.0", "{$taxonomy}_add_form")
else:
    #// 
    ...
    #// @param WP_Term $tag Current taxonomy term object.
    #//
    do_action_deprecated("edit_tag_form", Array(tag_), "3.0.0", "{$taxonomy}_edit_form")
# end if
#// 
...
#// @param string  $taxonomy Current taxonomy slug.
#//
do_action(str(taxonomy_) + str("_edit_form"), tag_, taxonomy_)
php_print("""
...
if current_user_can("delete_term", tag_.term_id):
    ...
    php_print(admin_url(wp_nonce_url(str("edit-tags.php?action=delete&taxonomy=") + str(taxonomy_) + str("&tag_ID=") + str(tag_.term_id), "delete-tag_" + tag_.term_id)))
    php_print("\">")
    _e("Delete")
    php_print("</a>\n       </span>\n   ")
# end if
