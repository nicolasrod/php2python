...
for case in Switch(action_):
    if case("editattachment"):
        ...
        # end if
        errors_ = media_upload_form_handler()
        if php_empty(lambda : errors_):
            location_ = "media.php"
            referer_ = wp_get_original_referer()
            if referer_:
                if False != php_strpos(referer_, "upload.php") or url_to_postid(referer_) == attachment_id_:
                    location_ = referer_
                # end if
            # end if
            if False != php_strpos(location_, "upload.php"):
                location_ = remove_query_arg("message", location_)
                location_ = add_query_arg("posted", attachment_id_, location_)
            elif False != php_strpos(location_, "media.php"):
                location_ = add_query_arg("message", "updated", location_)
            # end if
    ...
    if case("edit"):
        ...
        # end if
        if message_:
            php_print(str("<div id='message' class='") + str(class_) + str("'><p>") + str(message_) + str("</p></div>\n"))
        # end if
        php_print("""
        <div class=\"wrap\">
        ...
        php_print(get_media_item(att_id_, Array({"toggle": False, "send": False, "delete": False, "show_title": False, "errors": errors_[att_id_] if (not php_empty(lambda : errors_[att_id_])) else None})))
        php_print("""   </div>
        </div>
        """)
        submit_button(__("Update Media"), "primary", "save")
        php_print(" <input type=\"hidden\" name=\"post_id\" id=\"post_id\" value=\"")
        php_print(esc_attr(post_id_) if (php_isset(lambda : post_id_)) else "")
        php_print("\" />\n  <input type=\"hidden\" name=\"attachment_id\" id=\"attachment_id\" value=\"")
        php_print(esc_attr(att_id_))
