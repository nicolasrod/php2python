...
php_check_if_defined("_new_bundled_files")
_new_bundled_files = Array({"plugins/akismet/": "2.0", "themes/twentyten/": "3.0", "themes/twentyeleven/": "3.2", "themes/twentytwelve/": "3.5", "themes/twentythirteen/": "3.6", "themes/twentyfourteen/": "3.8", "themes/twentyfifteen/": "4.1", "themes/twentysixteen/": "4.4", "themes/twentyseventeen/": "4.7", "themes/twentynineteen/": "5.0", "themes/twentytwenty/": "5.3"})
#// 
...
#//
def update_core(from_=None, to=None, *args_):
    
    global wp_filesystem,_old_files,_new_bundled_files,wpdb
    php_check_if_defined("wp_filesystem","_old_files","_new_bundled_files","wpdb")
    set_time_limit(300)
    #// 
    ...
    for root in roots:
        if wp_filesystem.exists(from_ + root + "readme.html") and wp_filesystem.exists(from_ + root + "wp-includes/version.php"):
            distro = root
            break
        # end if
    # end for
    if (not distro):
        wp_filesystem.delete(from_, True)
    ...
    if (not wp_filesystem.copy(from_ + distro + "wp-includes/version.php", versions_file)):
        wp_filesystem.delete(from_, True)
    ...
    old_wp_version = PHP_GLOBALS["wp_version"]
    #// The version of WordPress we're updating from.
    development_build = False != php_strpos(old_wp_version + wp_version, "-")
    ...
    if (not mysql_compat) and (not php_compat):
        ...
        return php_new_class("WP_Error", lambda : WP_Error("php_not_compatible", php_sprintf(__("The update cannot be installed because WordPress %1$s requires PHP version %2$s or higher. You are running version %3$s."), wp_version, required_php_version, php_version) + php_update_message))
    elif (not mysql_compat):
        return php_new_class("WP_Error", lambda : WP_Error("mysql_not_compatible", php_sprintf(__("The update cannot be installed because WordPress %1$s requires MySQL version %2$s or higher. You are running version %3$s."), wp_version, required_mysql_version, mysql_version)))
    # end if
    #// Add a warning when the JSON PHP extension is missing.
    if (not php_extension_loaded("json")):
        return php_new_class("WP_Error", lambda : WP_Error("php_not_compatible_json", php_sprintf(__("The update cannot be installed because WordPress %1$s requires the %2$s PHP extension."), wp_version, "JSON")))
    # end if
    ...
    if php_function_exists("get_core_checksums"):
        ...
        working_dir_local = WP_CONTENT_DIR + "/upgrade/" + php_basename(from_) + distro
        checksums = get_core_checksums(wp_version, wp_local_package if (php_isset(lambda : wp_local_package)) else "en_US")
        if php_is_array(checksums) and (php_isset(lambda : checksums[wp_version])):
            checksums = checksums[wp_version]
            pass
        # end if
        if php_is_array(checksums):
            for file,checksum in checksums:
                ...
                if (not php_file_exists(ABSPATH + file)):
                    continue
                # end if
                if (not php_file_exists(working_dir_local + file)):
                    continue
                # end if
                if "." == php_dirname(file) and php_in_array(pathinfo(file, PATHINFO_EXTENSION), Array("html", "txt")):
                    continue
                # end if
                if php_md5_file(ABSPATH + file) == checksum:
                    skip[-1] = file
                else:
    ...
    if check_is_writable and "direct" == wp_filesystem.method:
        files_writable = php_array_filter(check_is_writable, Array(wp_filesystem, "is_writable"))
        if files_writable != check_is_writable:
            files_not_writable = php_array_diff_key(check_is_writable, files_writable)
            for relative_file_not_writable,file_not_writable in files_not_writable:
                #// If the writable check failed, chmod file to 0644 and try again, same as copy_dir().
                wp_filesystem.chmod(file_not_writable, FS_CHMOD_FILE)
                if wp_filesystem.is_writable(file_not_writable):
                    files_not_writable[relative_file_not_writable] = None
                # end if
            # end for
            #// Store package-relative paths (the key) of non-writable files in the WP_Error object.
            error_data = php_array_keys(files_not_writable) if php_version_compare(old_wp_version, "3.7-beta2", ">") else ""
            if files_not_writable:
                return php_new_class("WP_Error", lambda : WP_Error("files_not_writable", __("The update cannot be installed because we will be unable to copy some files. This is usually due to inconsistent file permissions."), php_implode(", ", error_data)))
            # end if
        # end if
    # end if
    #// This filter is documented in wp-admin/includes/update-core.php
    ...
    result = _copy_dir(from_ + distro, to, skip)
    if is_wp_error(result):
        result = php_new_class("WP_Error", lambda : WP_Error(result.get_error_code(), result.get_error_message(), php_substr(result.get_error_data(), php_strlen(to))))
    # end if
    #// Since we know the core files have copied over, we can now copy the version file.
    if (not is_wp_error(result)):
        if (not wp_filesystem.copy(from_ + distro + "wp-includes/version.php", to + "wp-includes/version.php", True)):
            wp_filesystem.delete(from_, True)
            result = php_new_class("WP_Error", lambda : WP_Error("copy_failed_for_version_file", __("The update cannot be installed because we will be unable to copy some files. This is usually due to inconsistent file permissions."), "wp-includes/version.php"))
        # end if
    ...
    failed = Array()
    if (php_isset(lambda : checksums)) and php_is_array(checksums):
        for file,checksum in checksums:
            if "wp-content" == php_substr(file, 0, 10):
                continue
            # end if
            ...
            if "." == php_dirname(file) and php_in_array(pathinfo(file, PATHINFO_EXTENSION), Array("html", "txt")):
                ...
                continue
            # end if
            if php_file_exists(ABSPATH + file) and php_md5_file(ABSPATH + file) == checksum:
                skip[-1] = file
            else:
                failed[-1] = file
            # end if
    ...
    if (not php_empty(lambda : failed)):
        total_size = 0
        for file in failed:
            if php_file_exists(working_dir_local + file):
                total_size += filesize(working_dir_local + file)
            # end if
        # end for
        #// If we don't have enough free space, it isn't worth trying again.
        #// Unlikely to be hit due to the check in unzip_file().
        available_space = php_no_error(lambda: disk_free_space(ABSPATH))
        if available_space and total_size >= available_space:
            result = php_new_class("WP_Error", lambda : WP_Error("disk_full", __("There is not enough free disk space to complete the update.")))
        else:
            result = _copy_dir(from_ + distro, to, skip)
            if is_wp_error(result):
                result = php_new_class("WP_Error", lambda : WP_Error(result.get_error_code() + "_retry", result.get_error_message(), php_substr(result.get_error_data(), php_strlen(to))))
            # end if
        # end if
    # end if
    #// Custom content directory needs updating now.
    #// Copy languages.
    if (not is_wp_error(result)) and wp_filesystem.is_dir(from_ + distro + "wp-content/languages"):
        ...
        if (not php_no_error(lambda: php_is_dir(lang_dir))) and 0 == php_strpos(lang_dir, ABSPATH):
            #// If it's within the ABSPATH we can handle it here, otherwise they're out of luck.
            wp_filesystem.mkdir(to + php_str_replace(ABSPATH, "", lang_dir), FS_CHMOD_DIR)
            clearstatcache()
            pass
        # end if
        if php_no_error(lambda: php_is_dir(lang_dir)):
            wp_lang_dir = wp_filesystem.find_folder(lang_dir)
            if wp_lang_dir:
                result = copy_dir(from_ + distro + "wp-content/languages/", wp_lang_dir)
                if is_wp_error(result):
                    result = php_new_class("WP_Error", lambda : WP_Error(result.get_error_code() + "_languages", result.get_error_message(), php_substr(result.get_error_data(), php_strlen(wp_lang_dir))))
                # end if
            # end if
        # end if
    # end if
    ...
    #//
    if (not is_wp_error(result)) and (not php_defined("CORE_UPGRADE_SKIP_NEW_BUNDLED")) or (not CORE_UPGRADE_SKIP_NEW_BUNDLED):
        for file,introduced_version in _new_bundled_files:
            #// If a $development_build or if $introduced version is greater than what the site was previously running.
            if development_build or php_version_compare(introduced_version, old_wp_version, ">"):
                directory = "/" == file[php_strlen(file) - 1]
                type, filename = php_explode("/", file, 2)
                #// Check to see if the bundled items exist before attempting to copy them.
                if (not wp_filesystem.exists(from_ + distro + "wp-content/" + file)):
                    continue
                # end if
                if "plugins" == type:
                ...
                # end if
                if (not directory):
                    if (not development_build) and wp_filesystem.exists(dest + filename):
                        continue
                    # end if
                    if (not wp_filesystem.copy(from_ + distro + "wp-content/" + file, dest + filename, FS_CHMOD_FILE)):
                        result = php_new_class("WP_Error", lambda : WP_Error(str("copy_failed_for_new_bundled_") + str(type), __("Could not copy file."), dest + filename))
                    # end if
                else:
                    if (not development_build) and wp_filesystem.is_dir(dest + filename):
                        continue
                    # end if
                    wp_filesystem.mkdir(dest + filename, FS_CHMOD_DIR)
                    _result = copy_dir(from_ + distro + "wp-content/" + file, dest + filename)
                    #// If a error occurs partway through this final step, keep the error flowing through, but keep process going.
                    if is_wp_error(_result):
                        if (not is_wp_error(result)):
                            result = php_new_class("WP_Error", lambda : WP_Error())
                        # end if
                        result.add(_result.get_error_code() + str("_") + str(type), _result.get_error_message(), php_substr(_result.get_error_data(), php_strlen(dest)))
                    # end if
                # end if
            # end if
        # end for
    ...
    # end if
    #// Remove old files.
    for old_file in _old_files:
        old_file = to + old_file
        if (not wp_filesystem.exists(old_file)):
            continue
        # end if
        #// If the file isn't deleted, try writing an empty string to the file instead.
        if (not wp_filesystem.delete(old_file, True)) and wp_filesystem.is_file(old_file):
...
#//
def _copy_dir(from_=None, to=None, skip_list=Array(), *args_):
    ...
    for filename,fileinfo in dirlist:
        ...
        if "f" == fileinfo["type"]:
            if (not wp_filesystem.copy(from_ + filename, to + filename, True, FS_CHMOD_FILE)):
                ...
                if (not wp_filesystem.copy(from_ + filename, to + filename, True, FS_CHMOD_FILE)):
                    return php_new_class("WP_Error", lambda : WP_Error("copy_failed__copy_dir", __("Could not copy file."), to + filename))
                # end if
            # end if
        elif "d" == fileinfo["type"]:
            if (not wp_filesystem.is_dir(to + filename)):
                if (not wp_filesystem.mkdir(to + filename, FS_CHMOD_DIR)):
                    return php_new_class("WP_Error", lambda : WP_Error("mkdir_failed__copy_dir", __("Could not create directory."), to + filename))
                # end if
            # end if
            #// 
            ...
            for skip_item in skip_list:
                ...
                # end if
            # end for
            result = _copy_dir(from_ + filename, to + filename, sub_skip_list)
            if is_wp_error(result):
                return result
            # end if
        # end if
...
#//
def _redirect_to_about_wordpress(new_version=None, *args_):
    
    global wp_version,pagenow,action
    php_check_if_defined("wp_version","pagenow","action")
    if php_version_compare(wp_version, "3.4-RC1", ">="):
        return
    # end if
    ...
    if "update-core.php" != pagenow:
        return
    # end if
    if "do-core-upgrade" != action and "do-core-reinstall" != action:
        return
    # end if
    ...
    show_message("<span class=\"hide-if-no-js\">" + php_sprintf(__("Welcome to WordPress %1$s. You will be redirected to the About WordPress screen. If not, click <a href=\"%2$s\">here</a>."), new_version, "about.php?updated") + "</span>")
    show_message("<span class=\"hide-if-js\">" + php_sprintf(__("Welcome to WordPress %1$s. <a href=\"%2$s\">Learn more</a>."), new_version, "about.php?updated") + "</span>")
    php_print("</div>")
...
#// @global WP_Filesystem_Base $wp_filesystem
#//
def _upgrade_422_remove_genericons(*args_):
    
    global wp_theme_directories,wp_filesystem
    php_check_if_defined("wp_theme_directories","wp_filesystem")
    #// A list of the affected files using the filesystem absolute paths.
    affected_files = Array()
    #// Themes.
    for directory in wp_theme_directories:
        affected_theme_files = _upgrade_422_find_genericons_files_in_folder(directory)
        affected_files = php_array_merge(affected_files, affected_theme_files)
    # end for
    #// Plugins.
    affected_plugin_files = _upgrade_422_find_genericons_files_in_folder(WP_PLUGIN_DIR)
    affected_files = php_array_merge(affected_files, affected_plugin_files)
    for file in affected_files:
        gen_dir = wp_filesystem.find_folder(trailingslashit(php_dirname(file)))
        if php_empty(lambda : gen_dir):
            continue
        # end if
        #// The path when the file is accessed via WP_Filesystem may differ in the case of FTP.
        remote_file = gen_dir + php_basename(file)
        if (not wp_filesystem.exists(remote_file)):
            continue
        # end if
        if (not wp_filesystem.delete(remote_file, False, "f")):
...
#// @return array
#//
def _upgrade_422_find_genericons_files_in_folder(directory=None, *args_):
    
    directory = trailingslashit(directory)
    files = Array()
    if php_file_exists(str(directory) + str("example.html")) and False != php_strpos(php_file_get_contents(str(directory) + str("example.html")), "<title>Genericons</title>"):
        files[-1] = str(directory) + str("example.html")
    # end if
    dirs = glob(directory + "*", GLOB_ONLYDIR)
    if dirs:
        for dir in dirs:
            files = php_array_merge(files, _upgrade_422_find_genericons_files_in_folder(dir))
        # end for
    # end if
    return files
...
#// @since 4.4.0
#//
def _upgrade_440_force_deactivate_incompatible_plugins(*args_):
