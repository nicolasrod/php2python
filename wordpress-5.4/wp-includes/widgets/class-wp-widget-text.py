...
class WP_Widget_Text(WP_Widget):
    ...
    def __init__(self):
        ...
        widget_ops_ = Array({"classname": "widget_text", "description": __("Arbitrary text."), "customize_selective_refresh": True})
        control_ops_ = Array({"width": 400, "height": 350})
        super().__init__("text", __("Text"), widget_ops_, control_ops_)
    # end def __init__
    #// 
    ...
    #//
    def is_legacy_instance(self, instance_=None):
        
        
        #// Legacy mode when not in visual mode.
        if (php_isset(lambda : instance_["visual"])):
            return (not instance_["visual"])
        # end if
        #// Or, the widget has been added/updated in 4.8.0 then filter prop is 'content' and it is no longer legacy.
        if (php_isset(lambda : instance_["filter"])) and "content" == instance_["filter"]:
            return False
        # end if
        #// If the text is empty, then nothing is preventing migration to TinyMCE.
        if php_empty(lambda : instance_["text"]):
            return False
        # end if
        wpautop_ = (not php_empty(lambda : instance_["filter"]))
        has_line_breaks_ = False != php_strpos(php_trim(instance_["text"]), "\n")
        #// If auto-paragraphs are not enabled and there are line breaks, then ensure legacy mode.
        if (not wpautop_) and has_line_breaks_:
            return True
        # end if
        #// If an HTML comment is present, assume legacy mode.
        if False != php_strpos(instance_["text"], "<!--"):
            return True
        # end if
        ...
        php_no_error(lambda: doc_.loadhtml(php_sprintf("<!DOCTYPE html><html><head><meta charset=\"%s\"></head><body>%s</body></html>", esc_attr(get_bloginfo("charset")), instance_["text"])))
        libxml_use_internal_errors(errors_)
        body_ = doc_.getelementsbytagname("body").item(0)
        #// See $allowedposttags.
        safe_elements_attributes_ = Array({"strong": Array(), "em": Array(), "b": Array(), "i": Array(), "u": Array(), "s": Array(), "ul": Array(), "ol": Array(), "li": Array(), "hr": Array(), "abbr": Array(), "acronym": Array(), "code": Array(), "dfn": Array(), "a": Array({"href": True})}, {"img": Array({"src": True, "alt": True})})
        safe_empty_elements_ = Array("img", "hr", "iframe")
        for element_ in body_.getelementsbytagname("*"):
            #// @var DOMElement $element
            tag_name_ = php_strtolower(element_.nodeName)
            #// If the element is not safe, then the instance is legacy.
            if (not (php_isset(lambda : safe_elements_attributes_[tag_name_]))):
                return True
            # end if
            #// If the element is not safely empty and it has empty contents, then legacy mode.
            if (not php_in_array(tag_name_, safe_empty_elements_, True)) and "" == php_trim(element_.textContent):
                return True
            # end if
            #// If an attribute is not recognized as safe, then the instance is legacy.
            for attribute_ in element_.attributes:
                #// @var DOMAttr $attribute
                attribute_name_ = php_strtolower(attribute_.nodeName)
                if (not (php_isset(lambda : safe_elements_attributes_[tag_name_][attribute_name_]))):
                    return True
                # end if
            # end for
        # end for
    ...
    def widget(self, args_=None, instance_=None):
        ...
        text_ = instance_["text"] if (not php_empty(lambda : instance_["text"])) else ""
        is_visual_text_widget_ = (not php_empty(lambda : instance_["visual"])) and (not php_empty(lambda : instance_["filter"]))
        #// In 4.8.0 only, visual Text widgets get filter=content, without visual prop; upgrade instance props just-in-time.
        if (not is_visual_text_widget_):
            is_visual_text_widget_ = (php_isset(lambda : instance_["filter"])) and "content" == instance_["filter"]
        # end if
        if is_visual_text_widget_:
            instance_["filter"] = True
            instance_["visual"] = True
        # end if
        #// 
        ...
        #//
        widget_text_do_shortcode_priority_ = has_filter("widget_text", "do_shortcode")
        should_suspend_legacy_shortcode_support_ = is_visual_text_widget_ and False != widget_text_do_shortcode_priority_
        if should_suspend_legacy_shortcode_support_:
            remove_filter("widget_text", "do_shortcode", widget_text_do_shortcode_priority_)
        # end if
        ...
        #//
        text_ = apply_filters("widget_text", text_, instance_, self)
        if is_visual_text_widget_:
            #// 
            ...
            #//
            text_ = apply_filters("widget_text_content", text_, instance_, self)
        else:
            #// Now in legacy mode, add paragraphs and line breaks when checkbox is checked.
            if (not php_empty(lambda : instance_["filter"])):
                text_ = wpautop(text_)
            # end if
            #// 
            ...
            #//
            if has_filter("widget_text_content", "do_shortcode") and (not widget_text_do_shortcode_priority_):
                if (not php_empty(lambda : instance_["filter"])):
                    text_ = shortcode_unautop(text_)
                # end if
                text_ = do_shortcode(text_)
            # end if
        # end if
        #// Restore post global.
    ...
    #//
    def inject_video_max_width_style(self, matches_=None):
    ...
    def update(self, new_instance_=None, old_instance_=None):
        ...
        new_instance_ = wp_parse_args(new_instance_, Array({"title": "", "text": "", "filter": False, "visual": None}))
        instance_ = old_instance_
        instance_["title"] = sanitize_text_field(new_instance_["title"])
        if current_user_can("unfiltered_html"):
            instance_["text"] = new_instance_["text"]
        else:
            instance_["text"] = wp_kses_post(new_instance_["text"])
        # end if
        instance_["filter"] = (not php_empty(lambda : new_instance_["filter"]))
        #// Upgrade 4.8.0 format.
        if (php_isset(lambda : old_instance_["filter"])) and "content" == old_instance_["filter"]:
            instance_["visual"] = True
        # end if
        if "content" == new_instance_["filter"]:
            instance_["visual"] = True
        # end if
        if (php_isset(lambda : new_instance_["visual"])):
            instance_["visual"] = (not php_empty(lambda : new_instance_["visual"]))
        # end if
        #// Filter is always true in visual mode.
        if (not php_empty(lambda : instance_["visual"])):
            instance_["filter"] = True
        # end if
        return instance_
    # end def update
    ...
    def form(self, instance_=None):
        ...
        instance_ = wp_parse_args(instance_, Array({"title": "", "text": ""}))
        php_print("     ")
        if (not self.is_legacy_instance(instance_)):
            php_print("         ")
            ...
            text_ = apply_filters("the_editor_content", instance_["text"], default_editor_)
            #// Reset filter addition.
            if user_can_richedit():
                remove_filter("the_editor_content", "format_for_editor")
            # end if
            #// Prevent premature closing of textarea in case format_for_editor() didn't apply or the_editor_content filter did a wrong thing.
            escaped_text_ = php_preg_replace("#</textarea#i", "&lt;/textarea", text_)
            php_print("         <input id=\"")
            ...
            """)
            if (not (php_isset(lambda : instance_["visual"]))):
                php_print("                 <p>")
            ...
            php_print(self.get_field_name("text"))
            php_print("\">")
            php_print(esc_textarea(instance_["text"]))
            php_print("""</textarea>
            </p>
            <p>
            <input id=\"""")
            ...
            checked((not php_empty(lambda : instance_["filter"])))
            php_print(" />&nbsp;<label for=\"")
            php_print(self.get_field_id("filter"))
            php_print("\">")
    ...
    def render_control_template_scripts(self):
        ...
        """)
        if (not php_in_array("text_widget_custom_html", dismissed_pointers_, True)):
        ...
        # end if
        php_print("\n           ")
        if (not php_in_array("text_widget_paste_html", dismissed_pointers_, True)):
