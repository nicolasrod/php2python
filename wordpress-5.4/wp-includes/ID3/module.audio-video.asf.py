...
class getid3_asf(getid3_handler):
    ...
    def codeclistobjecttypelookup(self, CodecListType=None):
        ...
        lookup = Array({1: "Video Codec", 2: "Audio Codec", 65535: "Unknown Codec"})
        return lookup[CodecListType] if (php_isset(lambda : lookup[CodecListType])) else "Invalid Codec Type"
    ...
    def guidname(self, GUIDstring=None):
        ...
        GUIDarray = Array()
        if php_empty(lambda : GUIDarray):
            GUIDarray = self.knownguids()
        # end if
        return php_array_search(GUIDstring, GUIDarray)
    # end def guidname
    #// 
    #// @param int $id
    #// 
    ...
    def asfindexobjectindextypelookup(self, id=None):
        ...
        ASFIndexObjectIndexTypeLookup = Array()
        if php_empty(lambda : ASFIndexObjectIndexTypeLookup):
        ...
        # end if
        return ASFIndexObjectIndexTypeLookup[id] if (php_isset(lambda : ASFIndexObjectIndexTypeLookup[id])) else "invalid"
    ...
    def wmpicturetypelookup(self, WMpictureType=None):
        ...
        lookup = None
        if lookup == None:
            lookup = Array({3: "Front Cover", 4: "Back Cover", 0: "User Defined", 5: "Leaflet Page", 6: "Media Label", 7: "Lead Artist", 8: "Artist", 9: "Conductor", 10: "Band", 11: "Composer", 12: "Lyricist", 13: "Recording Location", 14: "During Recording", 15: "During Performance", 16: "Video Screen Capture", 18: "Illustration", 19: "Band Logotype", 20: "Publisher Logotype"})
            lookup = php_array_map((lambda str = None:  getid3_lib.iconv_fallback("UTF-8", "UTF-16LE", str)), lookup)
        # end if
        return lookup[WMpictureType] if (php_isset(lambda : lookup[WMpictureType])) else ""
    ...
    def metadatalibraryobjectdatatypelookup(self, id=None):
        ...
        lookup = Array({0: "Unicode string", 1: "BYTE array", 2: "BOOL", 3: "DWORD", 4: "QWORD", 5: "WORD", 6: "GUID"})
        return lookup[id] if (php_isset(lambda : lookup[id])) else "invalid"
