...
class ParagonIE_Sodium_Core_BLAKE2b(ParagonIE_Sodium_Core_Util):
    ...
    def new64(self, high_=None, low_=None):
        ...
        i64_ = php_new_class("SplFixedArray", lambda : SplFixedArray(2))
        i64_[0] = high_ & 4294967295
        i64_[1] = low_ & 4294967295
        return i64_
    # end def new64
    #// 
    ...
    def add64(self, x_=None, y_=None):
        ...
        l_ = x_[1] + y_[1] & 4294967295
        return self.new64(php_int(x_[0] + y_[0] + 1 if l_ < x_[1] else 0), php_int(l_))
    # end def add64
    #// 
    #// @internal You should not use this directly from another application
    #// 
    ...
    def add364(self, x_=None, y_=None, z_=None):
        ...
        return self.add64(x_, self.add64(y_, z_))
    # end def add364
    #// 
    #// @internal You should not use this directly from another application
    #// 
    ...
    def rotr64(self, x_=None, c_=None):
        ...
        if c_ < 32:
            #// @var int $h0
            h0_ = php_int(x_[0]) << c_ | php_int(x_[1]) & 1 << c_ - 1 << 32 - c_ >> 32 - c_
            #// @var int $l0
            l0_ = php_int(x_[1]) << c_
        else:
            #// @var int $h0
            h0_ = php_int(x_[1]) << c_ - 32
        # end if
        h1_ = 0
        c1_ = 64 - c_
        if c1_ < 32:
            #// @var int $h1
            h1_ = php_int(x_[0]) >> c1_
            #// @var int $l1
            l1_ = php_int(x_[1]) >> c1_ | php_int(x_[0]) & 1 << c1_ - 1 << 32 - c1_
        else:
            #// @var int $l1
            l1_ = php_int(x_[0]) >> c1_ - 32
        # end if
        return self.new64(h0_ | h1_, l0_ | l1_)
    # end def rotr64
    #// 
    #// @internal You should not use this directly from another application
    #// 
    ...
    #// @psalm-suppress MixedOperand
    #//
    def flatten64(self, x_=None):
        
        
        return php_int(x_[0] * 4294967296 + x_[1])
    # end def flatten64
    #// 
    #// @internal You should not use this directly from another application
    #// 
    ...
    def increment_counter(self, ctx_=None, inc_=None):
        ...
        #// # S->t[1] += ( S->t[0] < inc );
        if self.flatten64(ctx_[1][0]) < inc_:
    ...
    def contexttostring(self, ctx_=None):
        ...
        while i_ < 8:
            ...
            str_ += self.store32_le(ctxA_[i_][1])
            str_ += self.store32_le(ctxA_[i_][0])
            i_ += 1
        # end while
        #// # uint64_t t[2];
        #// # uint64_t f[2];
        ...
        while i_ < 3:
            ...
            str_ += self.store32_le(ctxA_[0][1])
            str_ += self.store32_le(ctxA_[0][0])
            str_ += self.store32_le(ctxA_[1][1])
            str_ += self.store32_le(ctxA_[1][0])
            i_ += 1
        # end while
        #// # uint8_t buf[2 * 128];
        str_ += self.splfixedarraytostring(ctx_[3])
        #// @var int $ctx4
        ctx4_ = php_int(ctx_[4])
        #// # size_t buflen;
        str_ += php_implode("", Array(self.inttochr(ctx4_ & 255), self.inttochr(ctx4_ >> 8 & 255), self.inttochr(ctx4_ >> 16 & 255), self.inttochr(ctx4_ >> 24 & 255), self.inttochr(ctx4_ >> 32 & 255), self.inttochr(ctx4_ >> 40 & 255), self.inttochr(ctx4_ >> 48 & 255), self.inttochr(ctx4_ >> 56 & 255)))
        #// # uint8_t last_node;
    ...
    def stringtocontext(self, string_=None):
        ...
        while i_ < 8:
            ...
            ctx_[0][i_] = SplFixedArray.fromarray(Array(self.load_4(self.substr(string_, i_ << 3 + 4, 4)), self.load_4(self.substr(string_, i_ << 3 + 0, 4))))
            i_ += 1
        # end while
        #// # uint64_t t[2];
        #// # uint64_t f[2];
        ...
        while i_ < 3:
            ...
            ctx_[i_][1] = SplFixedArray.fromarray(Array(self.load_4(self.substr(string_, 76 + i_ - 1 << 4, 4)), self.load_4(self.substr(string_, 72 + i_ - 1 << 4, 4))))
            ctx_[i_][0] = SplFixedArray.fromarray(Array(self.load_4(self.substr(string_, 68 + i_ - 1 << 4, 4)), self.load_4(self.substr(string_, 64 + i_ - 1 << 4, 4))))
            i_ += 1
        # end while
        #// # uint8_t buf[2 * 128];
