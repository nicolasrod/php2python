...
def locale_stylesheet(*_args_):
    ...
    php_printf("<link rel=\"stylesheet\" href=\"%s\"%s media=\"screen\" />", stylesheet_, type_attr_)
...
def _custom_background_cb(*_args_):
    ...
    if (not background_) and (not color_):
        if is_customize_preview():
            php_printf("<style%s id=\"custom-background-css\"></style>", type_attr_)
        # end if
        return
    # end if
